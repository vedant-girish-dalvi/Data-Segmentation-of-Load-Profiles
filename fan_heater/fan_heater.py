# Vedant Girish Dalvi (EM-PE/EEY1)
...
def insert_zero_current_rows(start_time:float, end_time:float, filepath:str, output_file:str):
    ...
    for index, row in df.iterrows():
        ...
        if first_row_start_time != start:
            if CR_STATUS == False:# Create a new row with Mode as "Zero" and other values same as the previous row
                new_row = df.loc[0].copy()
                new_row['Sr. No.'] = current_number - 1
                new_row['Mode'] = 'Zero Current'
                new_row['Voltage Peak (V)'] = 0
            ...
            if CR_STATUS == False:
                ...
                last_row['Voltage Peak (V)'] = 0
                last_row['Current Peak (A)'] = 0
                last_row['Start Resistance (ohm)'] = 0
                last_row['End Resistance (ohm)'] = 0
                last_row['Start time (s)'] = last_row_end_time
                last_row['End time(s)'] = end
                last_row['Time (s)'] = end - last_row_end_time
                # Insert the last row at the end
...
def find_CR_segments(time, current, voltage, resistance, threshold):
    ...
    if start_time is not None:
        ...
        seq_count += 1
        segments.append((seq_count, segment_type, max_voltage, max_cr_current, end_resistance, last_resistance_value, cc_cr_time_values[0], end_time, end_time - cc_cr_time_values[0]))
...
def process_datafile(excel_file, threshold=ZERO_THRESHOLD):              #THRESHOLD : Change as per waveform
...
if __name__ == "__main__":
    ...
    excel_file_path = "fanheater-2023-08-25-10-42-58-Transient-Scope.xlsx"
